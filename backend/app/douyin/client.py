...
from app.douyin.signer import get_a_bogus, get_web_id
from app.douyin.types import PublishTimeType, SearchChannelType, SearchSortType
...
class DouyinClient:
        ...
        *,
        ...
        if sort_type != SearchSortType.general or publish_time != PublishTimeType.unlimited:
            params["filter_selected"] = json.dumps(
                {"sort_type": str(sort_type.value), "publish_time": str(publish_time.value)}
            )
    ...
    async def get_liked(
        ...
        return await self.get(
            ...
            {"aid": "6383", "sec_user_id": sec_user_id, "max_cursor": cursor, "count": count},
            headers,
        )

    async def get_collected(self, cursor: int | str, count: int) -> dict[str, Any]:
        headers = copy.copy(self.headers)
        headers["Referer"] = "https://www.douyin.com/user/self?showTab=favorite_collection"
