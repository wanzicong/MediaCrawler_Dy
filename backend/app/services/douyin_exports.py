...
def build_comments_export(
    ...
    selected = list(dict.fromkeys(value.strip() for value in aweme_ids if value.strip()))
    awemes = session.exec(
        select(DouyinAweme)
        .where(
...
def build_subtitles_export(
    ...
    *,
    ...
    missing = [aweme_id for aweme_id in selected if aweme_id not in {x.aweme_id for x in completed}]
...
def _subtitle_content(subtitle: DouyinSubtitle, export_format: str) -> str:
    ...
    if not segments and subtitle.full_text:
        segments = [{"start": 0.0, "end": subtitle.duration_seconds, "text": subtitle.full_text}]
