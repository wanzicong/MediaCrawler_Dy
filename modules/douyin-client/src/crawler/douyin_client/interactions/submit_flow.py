...
class SubmitFlow:
    ...
    两个入口方法（``_fill_and_submit`` / ``_wait_comment_submission``）为类方法，
    ...
    async def _report(
        ...
        """若提供了步骤回调，则上报一个执行步骤（本类内部专用）。"""
        if step_callback is not None:
            await step_callback(page, step, detail)
    ...
    async def _fill_and_submit(
        ...
        *,
        ...
        if expected_aweme_id:
            PageController._assert_video_page(page, expected_aweme_id)
        if (
            expected_reply_context is not None
            and not await CommentLocator._reply_context_is_active(
        ...
        submit = await PageController._find_submit_control(page, editor)
        ...
        try:
            if require_comment_confirmation:
                ...
                async with page.expect_response(
                    lambda response: ResponseInspector._is_comment_publish_response(
                        response
                    ),
                    timeout=12_000,
                ) as response_info:
                    await submit.scroll_into_view_if_needed()
                    submitted = await PageController._dispatch_comment_submit(
                        page, submit
                    )
                    if not submitted:
                        raise InteractionExecutionError(
                ...
                if (
                    ...
                    and not ResponseInspector._request_targets_reply(
                        response.request,
                ...
                payload = await ResponseInspector._safe_json(response)
                status_code = ResponseInspector._platform_status_code(payload)
                if status_code not in (None, 0, "0"):
                    raise InteractionExecutionError(
                        "platform_rejected",
                        ResponseInspector._platform_error_message(
                            payload, prefix="抖音未接受评论请求"
                        ),
                        affects_account_health=True,
                    )
                platform_id = ResponseInspector._result_id(payload)
