...
页面级查询/点击基元（可见元素查找、坐标点击、输入框判空、文案探测）由
``crawler.browser.page.primitives`` 统一提供、本模块直接导入。
此处仅保留带抖音语义的操作：评论发布请求的触发与观测、发送控件查找、
页面是否停留在目标视频页的断言，以及互动页面的筛选。方法已全部去掉下划线：
它们是本包与 ``tests/browser/`` 的协作面，不是私有实现。
"""
...
class PageController:
    ...
    @staticmethod
    async def dispatch_comment_submit(page: Page, control: Locator) -> bool:
        ...
        def observe_request(request: Any) -> None:
            ...
            if ResponseInspector.is_comment_publish_request(request):
    ...
    @staticmethod
    async def find_submit_control(page: Page, editor: Locator) -> Locator | None:
    ...
    @staticmethod
    def assert_video_page(page: Page, aweme_id: str) -> None:
    ...
    @staticmethod
    def interaction_pages(page: Page, *, aweme_id: str | None) -> list[Page]:
