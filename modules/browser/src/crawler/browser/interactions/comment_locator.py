...
编辑器的查找；通用 DOM 基元来自 ``crawler.browser.page.primitives``，选择器见
``selectors``。方法已全部去掉下划线：它们是本包与 ``tests/browser/`` 的协作面，
不是私有实现。
"""
...
class CommentLocator:
            ...
            for root in roots:
                ...
                for candidates_locator, require_unique in candidates:
                    try:
                        ...
                        continue
            if await cls.scroll_comment_list(page):
                no_scroll_progress = 0
            else:
    ...
    @classmethod
    async def scroll_comment_list(cls, page: Page) -> bool:
    ...
    @classmethod
    async def ensure_comment_list_active(cls, page: Page) -> bool:
        """确保评论列表处于可见激活状态：必要时点击评论标签并逐方式验证。

        返回：
            评论列表已可见返回 True，否则 False。
        """
        if await cls.find_visible_comment_surface(page, timeout=300) is not None:
            return True
        control = await dom.find_visible(page, COMMENT_TAB_SELECTORS, timeout=3_000)
        if control is None:
            return False
        ...
        for activate in activators:
            try:
                ...
                continue
            if await cls.find_visible_comment_surface(page, timeout=1_500) is not None:
                return True
        return False

    @classmethod
    async def find_visible_comment_surface(
        ...
        if comment_list is not None:
            return comment_list
        return await dom.find_visible(page, COMMENT_ITEM_SELECTORS, timeout=timeout)

    @classmethod
    async def open_reply_editor(
        cls,
        page: Page,
        target: Locator,
        ...
        for activate in activators:
            ...
            while asyncio.get_running_loop().time() < deadline:
                ...
                if editor is not None and await cls.reply_context_is_active(
    ...
    @staticmethod
    async def reply_context_is_active(
