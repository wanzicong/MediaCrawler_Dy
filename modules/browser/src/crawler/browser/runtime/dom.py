...
异常约定：本模块只抛出 browser 集成层的中立异常（``base.errors``），
绝不抛出业务/领域层的异常类型。
"""
...
from crawler.browser.base.errors import BrowserAutomationError
...
async def visible_page_message(
...
async def scroll_container_to_bottom(
    page: Page, anchor: Locator | None
) -> bool:
    """把锚点所在最近的真实可滚动容器滚到底。
