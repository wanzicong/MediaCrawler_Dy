...
属于浏览器集成层的编排入口，是上层业务（登录、采集、互动）获取 Playwright
Page 的统一入口。本类只做编排——把远程/本地分支交给连接器与启动器、页面获取
交给页面策略，自身不再实现进程管理或协议细节，绝不回退到 launch 方式。
"""
...
from crawler.browser.base.modes import DouyinBrowserMode, coerce_browser_mode
...
from crawler.browser.runtime.launcher import LocalChromeLauncher
...
class CDPBrowserSession:
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            stealth_path = Path(__file__).resolve().parents[1] / "resources" / "stealth.js"
    ...
    async def _acquire_page(self) -> None:
        ...
        await policy.acquire(self.context)
        self.page = policy.page
