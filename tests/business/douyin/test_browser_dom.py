...
这些基元从 douyin-client 下沉到 ``crawler.browser.runtime.dom``，与站点无关。
此处只覆盖各函数自身的边界行为（坐标点击、输入框判空、可见文案探测、
滚动兜底、evaluate 重试与对话框自动关闭），不带任何抖音语义。
"""
...
def test_click_control_center_clicks_geometry_center() -> None:
    ...
    asyncio.run(dom.click_control_center(page, control))
...
def test_editor_is_empty_falls_back_to_text_content() -> None:
    ...
    assert asyncio.run(dom.editor_is_empty(editor)) is True
...
def test_editor_is_empty_reports_nonempty_and_empty_editor() -> None:
    ...
    assert asyncio.run(dom.editor_is_empty(filled)) is False
    assert asyncio.run(dom.editor_is_empty(empty)) is True
...
def test_wait_editor_empty_polls_until_clear() -> None:
    ...
    monkeypatch.setattr(dom, "editor_is_empty", empty)
    try:
        result = asyncio.run(dom.wait_editor_empty(editor, timeout_ms=1_000))
    finally:
        monkeypatch.undo()
...
def test_visible_page_message_returns_first_visible_match() -> None:
    ...
    result = asyncio.run(
        dom.visible_page_message(page, ("发送失败", "评论已发送"))
    )
...
def test_scroll_container_to_bottom_uses_anchor_evaluate() -> None:
    ...
    result = asyncio.run(dom.scroll_container_to_bottom(page, anchor))
...
def test_scroll_container_to_bottom_wheel_fallback_without_anchor() -> None:
    ...
    result = asyncio.run(dom.scroll_container_to_bottom(page, None))
...
def test_evaluate_stable_succeeds_on_first_try() -> None:
    ...
    assert asyncio.run(dom.evaluate_stable(page, "1 + 1")) == "stable"
...
def test_evaluate_stable_reraises_unrelated_error() -> None:
    ...
    with pytest.raises(PlaywrightError, match="some other failure"):
        asyncio.run(dom.evaluate_stable(page, "1 + 1"))
...
def test_auto_dismiss_dialogs_registers_and_returns_handler() -> None:
    ...
    handler = dom.auto_dismiss_dialogs(page)
...
def test_auto_dismiss_dialogs_runs_on_dismiss_callback() -> None:
    ...
    handler = dom.auto_dismiss_dialogs(page, on_dismiss=on_dismiss)
    asyncio.run(handler(dialog))
