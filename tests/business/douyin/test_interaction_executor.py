...
def test_executor_execute_accepts_both_legacy_account_and_neutral_connection(
    ...
    monkeypatch.setattr(
        "crawler.douyin_client.interactions.executor.CDPBrowserSession",
        FakeBrowserSession,
    )
    account = SimpleNamespace(
...
def test_message_submit_selectors_cover_real_douyin_arrow() -> None:
    ...
    assert "svg.e2e-send-msg-btn" in MESSAGE_SUBMIT_SELECTORS
    assert "svg.messageMsgInputpublishRedBtn" in MESSAGE_SUBMIT_SELECTORS
...
def test_open_reply_editor_walks_to_card_and_verifies_reply_context(
    ...
    context, result = asyncio.run(
        CommentLocator._open_reply_editor(page, nodes[0], request)
    )
...
def test_open_reply_editor_stops_before_ambiguous_comment_list() -> None:
    ...
    context, editor = asyncio.run(
        CommentLocator._open_reply_editor(AsyncMock(), target, request)
    )
...
def test_reply_context_accepts_legacy_comment_id_attribute() -> None:
    ...
    assert (
        asyncio.run(CommentLocator._reply_context_is_active(comment_card, "456")) is True
    )
...
def test_reply_publish_request_must_target_confirmed_top_level_comment() -> None:
    ...
    assert (
        ResponseInspector._request_targets_reply(
            request,
            target_comment_id="456",
            parent_comment_id="0",
        )
...
def test_comment_publish_response_accepts_current_endpoint_variants(url: str) -> None:
    ...
    assert ResponseInspector._is_comment_publish_response(response) is True
...
def test_comment_publish_response_rejects_reads_and_list_requests() -> None:
    ...
    assert ResponseInspector._is_comment_publish_response(response) is False
...
def test_comment_submit_falls_back_to_component_activation_when_click_is_lost() -> None:
    ...
    result = asyncio.run(PageController._dispatch_comment_submit(page, control))
...
def test_creator_message_waits_for_profile_and_requires_send_control(
    ...
    monkeypatch.setattr(SubmitFlow, "_fill_and_submit", submit)
    request = InteractionExecutionRequest(
...
def test_comment_submit_requires_publish_request_and_empty_editor(
    ...
    monkeypatch.setattr(
        PageController, "_find_submit_control", AsyncMock(return_value=submit)
    )
    dispatch = AsyncMock(return_value=True)
    monkeypatch.setattr(PageController, "_dispatch_comment_submit", dispatch)
    monkeypatch.setattr(SubmitFlow, "_wait_comment_submission", AsyncMock())

    result = asyncio.run(
        SubmitFlow._fill_and_submit(
            page,
            editor,
            "测试评论",
            None,
    ...
    dispatch.assert_awaited_once_with(page, submit)
...
def test_ten_comment_publish_success_payloads(
    ...
    monkeypatch.setattr(
        PageController, "_dispatch_comment_submit", AsyncMock(return_value=True)
    )
    wait_for_ui = AsyncMock()
    monkeypatch.setattr(SubmitFlow, "_wait_comment_submission", wait_for_ui)
...
def test_comment_submit_detects_platform_failure_toast() -> None:
    ...
    with pytest.raises(InteractionExecutionError) as captured:
        asyncio.run(
            SubmitFlow._wait_comment_submission(page, request_content="测试评论")
        )
...
def test_comment_submit_detects_sms_verification() -> None:
    ...
    with pytest.raises(InteractionExecutionError) as captured:
        asyncio.run(SubmitFlow._wait_comment_submission(page, request_content="测试评论"))
...
def test_comment_submit_requires_a_clickable_send_control(
    ...
    editor = AsyncMock()
    monkeypatch.setattr(
        PageController, "_find_submit_control", AsyncMock(return_value=None)
    )
...
def test_interaction_pages_exclude_unrelated_manual_tabs() -> None:
    ...
    assert PageController._interaction_pages(target, aweme_id="123") == [target]
...
def test_find_comment_target_skips_hidden_duplicate_text(
    ...
    result = asyncio.run(CommentLocator._find_comment_target(page, request))
...
def test_find_comment_target_prefers_stable_tooltip_comment_id(
    ...
    result = asyncio.run(CommentLocator._find_comment_target(MagicMock(), request))
...
def test_reply_to_comment_classifies_live_target_state(
    ...
    monkeypatch.setattr(
        CommentLocator, "_ensure_comment_list_active", AsyncMock(return_value=True)
    )
    monkeypatch.setattr(
        CommentLocator, "_find_comment_target", AsyncMock(return_value=None)
    )
    monkeypatch.setattr(
...
def test_reply_to_comment_submits_only_with_confirmed_reply_context(
    ...
    monkeypatch.setattr(
        CommentLocator, "_find_comment_target", AsyncMock(return_value=target)
    )
    monkeypatch.setattr(
        CommentLocator,
        "_open_reply_editor",
