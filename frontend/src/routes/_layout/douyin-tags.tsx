...
import { Film, Hash, RefreshCw, Search, Tags } from "lucide-react"
    ...
    <div className="space-y-6">
      <section className="overflow-hidden rounded-3xl border bg-gradient-to-br from-primary/10 via-card to-card p-6 shadow-sm md:p-8">
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-3">
          <Metric
            label="匹配标签"
            value={tagsQuery.data?.count ?? 0}
            icon={Hash}
          />
          <Metric
            label="本页关联视频"
            value={rows.reduce((total, item) => total + item.aweme_count, 0)}
            icon={Film}
          />
          <Metric
            label="本页关联任务"
            value={rows.reduce((total, item) => total + item.task_count, 0)}
            icon={Tags}
          />
        </div>
      </section>
